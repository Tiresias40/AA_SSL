...
public class ExactDominantSetSolver implements DominatingSetSolverInterface {
	...
	/**
	 * Check if a dominatingSet of defenders exit in the graph
	 * 
	 * @param maxSize : size of the dominatingSet
	 * @return boolean represent the result of the check
	 */
	protected boolean hasDominatingSetRecursive(int maxSize) {
		...
		// Init dominating set with maxSize first vertices
		dominatingSet.clear();
		...
		// runs until all combinations have been tested or a good one has been
		// found
		while (true) {
			...
			if (goodCombination
					&& BasicGraphBuilder.allIntersected(dominatingSet)) {
				return true;
			}
	...
	/**
	 * Calculate a dominating set if it exist
	 * 
	 * @return boolean represent if a dominating set exist
	 */
	private boolean calculateDominatingSet() {
		...
		if (indexCurrInDef + 1 >= g.getDefendersVertices().size()) {
			...
			// We take the next element and test it
				...
				// if the last element of dominating set is equal to the one we
				// were processing, it means we need to go higher in the
				// combination to incr
				if (g.getDefendersVertices().get(indexCurrInDef)
						.equals(dominatingSet.lastElement()))
					ret = calculateDominatingSet();
				// if the element we were processing is bigger than the last of
				// dominating set
				// we take the one just after the value of dominating set (we
				// incr only by one)
				int indexLastOfDSInDef = g.getDefendersVertices().indexOf(
						dominatingSet.lastElement());
				if (indexCurrInDef > indexLastOfDSInDef + 1)
					current = g.getDefendersVertices().get(
							indexLastOfDSInDef + 1);
			}
